...
CREATE UNIQUE INDEX idx_transfer_lists_owner_name_unique
ON public.transfer_lists(owner_id, lower(name))
WHERE deleted_at IS NULL;

CREATE INDEX idx_transfer_lists_owner_updated
...
CREATE TABLE public.transfer_list_members (
...
CREATE INDEX idx_transfer_list_members_list_created ON public.transfer_list_members(list_id, created_at);
CREATE INDEX idx_transfer_list_members_user ON public.transfer_list_members(member_user_id);
...
-- Transfer list policies
CREATE POLICY "Users can manage their own transfer lists."
ON public.transfer_lists FOR ALL
USING (
  EXISTS (
    SELECT 1 FROM public.users
    WHERE users.id = transfer_lists.owner_id AND users.clerk_user_id = auth.uid()::text
  )
)
WITH CHECK (
  EXISTS (
